...
pbxProject.prototype.addSourceFile = function (path, opt, group) {
    ...
    console.log("addSourceFile is called" + "path:" + path + "group: " + group);
...
pbxProject.prototype.addToPbxGroup = function (file, groupKey) {
    ...
    //var group = this.getPBXGroupByPath(groupKey);
    //console.log("group: " + group + " file " + file + " groupKey " + groupKey);
    if (group && group.children !== undefined) {
        console.log("group.children is valid");
        if (typeof file === 'string') {
            console.log("group.children is valid 11");
            ...
            //File Object
            console.log("group.children is valid 12");

            group.children.push(pbxGroupChild(file));
        }
    } else {
        console.log("group.children is invalid");
    }
}
...
pbxProject.prototype.getPBXGroupByPath = function(path) {
    console.log("begin getPBXGroupByPath " + path);

    var groups = this.hash.project.objects['PBXGroup'];
    for (var key in groups) {
        var group = groups[key];
        if (group.path === path) {
            console.log("groupInfo", group, "path: ", group.path, "name: ", group.name);
            console.log("finish getPBXGroupByPath successful" + path);
            return group;
        }
    }

    console.log("finish getPBXGroupByPath failed" + path);
    return undefined;
};


pbxProject.prototype.getPBXGroupByKey = function(key) {
    console.log("begin getPBXGroupByKey " + key);

    var groups = this.hash.project.objects['PBXGroup'];
    for (var keyIter in groups) {
        var group = groups[keyIter];
        console.log("keyInfo", keyIter, keyIter.path, keyIter.name);

        console.log("groupInfo", group, "path: ", group.path, "name: ", group.name);
    }

    console.log("finish getPBXGroupByKey " + key);
...
pbxProject.prototype.addFile = function (path, group, opt) {
    ...
    // null is better for early errors
    if (this.hasFile(file.path))  {
        console.log("addSourceFile failed" + "path:" + file.path + " group: " + group);        
        return null;
    }

    file.fileRef = this.generateUuid();
    console.log("file.fileRef" + file.fileRef);
...
pbxProject.prototype.removeGroupFromPbxGroup = function (removeGroupKey, groupKey) {
    ...
    delete removeGroup;
    //this.removePbxGroupByKey(removeGroupKey);
}
